...
function ENT:Use(activator, caller)
	if activator:Team() == FACTION_ZOMBIE then return end
...
function RemoveFromBackpack()
	...
	elseif option == "singledrop" then
		...
		if not myClass then myClass = "ammo_" .. tostring(removalTbl.ammo[1]) end
		...
		if myType == "ammo" then
			if backEnt.contents.ammo[removalTbl.ammo[1]] < PNRP.Items[myClass].Energy then
				...
				if backEnt.contents.ammo[removalTbl.ammo[1]] <= 0 then backEnt.contents.ammo[removalTbl.ammo[1]] = nil end
			end
		else
			local myEnt = PNRP.Items[myClass].Create(ply, PNRP.Items[myClass].Ent, backEnt:GetPos() + Vector(0, 0, 20))
			backEnt.contents.inv[myClass] = backEnt.contents.inv[myClass] - 1
			if backEnt.contents.inv[myClass] <= 0 then backEnt.contents.inv[myClass] = nil end
		end
		...
		for k, v in pairs(backEnt.contents.inv) do
			if PNRP.Items[k].Type == "weapon" then
				...
				if subtractItem then
					...
					if backEnt.contents.inv[k] <= 0 then backEnt.contents.inv[k] = nil end
				end
			end
		end

		for k, v in pairs(backEnt.contents.ammo) do
			if k == "slam" then ply:Give("weapon_pnrp_charge") end
			ply:GiveAmmo(v, k)
	...
	elseif option == "takeall" then
		...
		for k, v in pairs(backEnt.contents.ammo) do
			...
			if weightCap > currentCarry + ciWeight then
				if v / PNRP.Items["ammo_" .. k].Energy > 0 then PNRP.AddToInventory(ply, "ammo_" .. k, math.floor(v / PNRP.Items["ammo_" .. k].Energy)) end
			...
			elseif weightCap > currentCarry + sngWeight then
				local weightLeft = weightCap - currentCarry
				local canHave = math.floor(weightLeft / sngWeight)
				if v / PNRP.Items["ammo_" .. k].Energy > 0 then PNRP.AddToInventory(ply, "ammo_" .. k, canHave) end
	...
	if table.Count(backEnt.contents.inv) <= 0 and table.Count(backEnt.contents.ammo) <= 0 and backEnt.contents.res.scrap <= 0 and backEnt.contents.res.small <= 0 and backEnt.contents.res.chems <= 0 then backEnt:Remove() end
end
