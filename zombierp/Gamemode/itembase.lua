...
function PNRP.SearchItembase(ent)
	...
	if ent:IsVehicle() then
		for itemname, item in pairs(PNRP.Items) do
			if item.Model == model then return item end
		end
	end
	...
	if PNRP.Items[class] then
		...
		if class == "prop_physics" then
			for itemname, item in pairs(PNRP.Items) do
				if item.Ent == "prop_physics" and item.Model == model and ent.crafted then return item end
			end
		end
	end
	return nil
...
function PNRP.FindWepItem(model)
	...
	if model == "models/Weapons/V_hands.mdl" or model == "models/weapons/v_hands.mdl" then fixedModel = "models/props_citizen_tech/transponder.mdl" end
	for itemname, item in pairs(PNRP.Items) do
		if fixedModel == item.Model then return item end
	end
	return nil
end
	...
	for itemid, amount in pairs(Tbl) do
		if PNRP.Items[itemid] then
			if (not foundItems) or (not foundItems[itemid]) or foundItems[itemid] < amount then
				if amount == 0 then
					if ply then ply:ChatPrint("You don't have a " .. PNRP.Items[itemid].Name .. ", which is required to build this.") end
				else
					if ply then ply:ChatPrint("You don't have enough " .. PNRP.Items[itemid].Name .. "s.  You require " .. tostring(amount) .. " to build this.") end
				end
				return false
			end
		end
		...
		if plUID == tostring(v:GetNetVar("Owner_UID", "None")) then
			...
			if item and Tbl[item["ID"]] then
				if Tbl[item["ID"]] > 0 then
					if v.iid then PNRP.DelPersistItem(v.iid) end
					v:Remove()
		...
		if amount ~= 0 then
			if plyInv[itemid] then
				...
				for i = 1, amount do
					...
					if result then PNRP.DelPersistItem(result[1]["iid"]) end
				end
			end
		end
	end
...
function PNRP.AddStatusItem(ply, ent, location)
	...
	if item.SaveState and IsValid(ent) then
		...
		if item.HP then ent:SetHealth(tonumber(item.HP)) end
		return iid
	end
	return false
end
...
function PNRP.CleanPersistItems()
	...
	for _, v in pairs(entTbl) do
		if v.iid then foundID[v.iid] = tostring(v:GetClass()) end
	end
	...
	if result then
		for _, v in pairs(result) do
			local iid = v["iid"]
			if not foundID[iid] then querySQL("DELETE FROM inventory_table WHERE iid=" .. tostring(iid)) end
		end
	end
end
--EOF