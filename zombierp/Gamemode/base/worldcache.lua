	...
	if PNRP.Items[theitem].Type == "tool" or PNRP.Items[theitem].Type == "misc" or PNRP.Items[theitem].Type == "vehicle" then
		...
		if ent then
			if ent.iid then
				...
				if iidResult then
					isIID = true
					if tostring(iidResult["location"]) ~= "world" then PNRP.SaveState(p, ent, "world") end
				end
			end
		end
...
function PNRP.CleanWorldAfterReturn(ply)
	...
	for k, v in pairs(ents.GetAll()) do
		...
		if ItemID ~= nil then
			...
			if tostring(v:GetNetVar("Owner_UID", "None")) == plUID then if myType == "vehicle" or myType == "tool" or myType == "misc" then v:Remove() end end
		end
	end
end
--EOF