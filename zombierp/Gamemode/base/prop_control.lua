...
function AddToolBlockedProp(mdl)
	table.insert(ToolBlockedProps, mdl)
end
...
for k, item in pairs(PNRP.Items) do
	if not item.UnBlock then AddBannedProp(item.Model) end
end

function PNRP.GetBannedPropsList()
	local tbl = {}
	if not file.IsDir("PostNukeRP", "DATA") then file.CreateDir("PostNukeRP") end
...
function PNRP.PropProtect_RemoveItem(len, ply)
	...
	if switch == 1 then
		...
		if file.Exists("PostNukeRP/banned_props.txt", "DATA") then
			...
			if tbl ~= nil then
				for k, v in pairs(tbl) do
					if model == v then table.remove(tbl, k) end
				end
		...
		if file.Exists("PostNukeRP/allowed_props.txt", "DATA") then
			...
			for k, v in pairs(AllowedProps) do
				if model == v then table.remove(AllowedProps, k) end
			end
		else
			for k, v in pairs(AllowedProps) do
				if model == v then table.remove(AllowedProps, k) end
		...
		if getServerSetting("propSpawnpointProtection") == 1 then
			...
			for k, v in pairs(fountEnts) do
				...
				for i, ent in ipairs(found_ents) do
					...
					if myClass == "prop_physics" and not ent.ID and not ent.crafted then ent:Remove() end
					if ent:IsDoor() then ent:SetNetVar("Owner", "Unownable") end
				end
			end
		end
	end)
...
function GM:PlayerSpawnedVehicle(ply, ent)
	...
	if not item then
		ent:SetHealth(tonumber(75))
	elseif not item.HP then
		ent:SetHealth(tonumber(75))
	end
end
...
function PuntCheck(ply, ent)
	if not (ply:IsAdmin() and getServerSetting("adminTouchAll") == 1) then
		if getServerSetting("AllowPunt") == 1 then
			...
			local Item = PNRP.SearchItembase(ent)
			if Item ~= nil then if Item.AllowPunt == true then return true end end
			return false
		end
	end
	return true
...
function PlayerPickupObject(ply, ent)
	...
	if ply:KeyPressed(IN_USE) then doPickup = true end
	if doPickup then
		...
		if canPickup then ply:PickupObject(ent) end
	end
end
...
function PickupCheck(ply, ent)
	...
	if string.find(ent:GetClass(), "unc_") == 2 then return false end
	if string.find(ent:GetClass(), "dynam") == 6 then return false end
	local searchPos
	searchPos = string.find(ent:GetClass(), "door_")
	if not searchPos then searchPos = 0 end
	if searchPos > 1 then return false end
	...
	if ent.GetPlayer and type(ent.GetPlayer) == "function" then if ply == ent:GetPlayer() then return true end end
	...
	if ent.GetPlayer and type(ent.GetPlayer) == "function" and not ownerEnt then ownerEnt = ent:GetPlayer() or nil end
	if ownerEnt then
...
function ToolCheck(ply, tr, toolmode)
	...
	if SAT then
		...
		if not SAT_CHECK then return false end
	end
	...
	local searchPos
	--Blocks weapons
	if string.find(ent:GetClass(), "ep_") == 2 then return false end
	--Blocks Func Ents
	if string.find(ent:GetClass(), "unc_") == 2 then return false end
	...
	if string.find(ent:GetClass(), "pc_") == 2 then
		if string.find(tostring(ent:GetClass()), "turret") then
			if toolmode == "weld" or toolmode == "weld_ez" or toolmode == "easy_precision" or toolmode == "nocollide" then
			else
				return false
			end
		else
	...
	if ent:IsVehicle() then
		if toolmode == "nocollide_world" or toolmode == "AdvBallsocket" or toolmode == "ballsocket_adv" or toolmode == "ballsocket_ez" then
	...
	if DoClassToolCheck then if not (toolmode == "remover" or toolmode == "weld" or toolmode == "weld_ez" or toolmode == "easy_precision" or toolmode == "duplicator" or toolmode == "adv_duplicator" or toolmode == "weld_ez2" or toolmode == "nocollide") then return false end end
	...
	if DoToolCheck then if not (toolmode == "weld" or toolmode == "weld_ez" or toolmode == "easy_precision" or toolmode == "weld_ez2" or toolmode == "nocollide") then return false end end
	--if owner == "None" or owner == "World" then return true end
	if owner == "None" or owner == "World" then return true end
end
...
function PNRP.PlayerExitVehicle(ply, vehicle)
	if vehicle.ExitAng then local origin = vehicle:GetPos() end
end
...
function PNRP.removeCarSeats(ent)
	...
	for _, seat in pairs(seats) do
		if seat.Entity[2].Entity.seat == 1 then seat.Entity[2].Entity:Remove() end
	end
end
