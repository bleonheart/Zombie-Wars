...
function SetItmName(ply, cmd, args)
	...
	local ent = tr.Entity
	if IsValid(ent) then if not ent:IsPlayer() then if tostring(ent:GetNetVar("Owner_UID", "None")) == PNRP:GetUID(ply) then if args[1] then ent:SetNetVar("name", args[1]) end end end end
end
...
function AmendCommunityInfo(cid, name, title, rank, lastlog, model, pid)
	...
	if name then values2 = values2 .. " nick=" .. SQLStr(name) end
	if rank then values = values .. " rank='" .. tostring(rank) .. "'" end
	if title then
...
function RemCommunityInfo(cid, pid)
	...
	result = querySQL(query)
	if result then rank = result[1]["rank"] end
...
function DelCommunity(cid)
	...
	if resultDip then
		for _, com in pairs(resultDip) do
			...
			for _, ocid in pairs(diplomacySplit) do
				local splitDip = string.Explode(",", ocid)
				if tonumber(splitDip[1]) ~= tonumber(cid) then dipTbl[splitDip[1]] = splitDip[2] end
			end
...
function GetCommunityTbl(cid)
	...
	--"CREATE TABLE community_table ( cid INTEGER PRIMARY KEY AUTOINCREMENT, cname varchar(255), res varchar(255), inv varchar(255), founded varchar(255) )"
	--"CREATE TABLE community_members ( pid int, cid int, rank int, title varchar(255) )"
	if cid == nil then --nil check for people no tin community
		cid = -1
	end
...
function AdmEditCom(ply, cmd, args)
	if ply:IsAdmin() then
		...
		if GetComtbl then tbl = GetComtbl end
		local wars = {}
		local allies = {}
...
function PlyInvComm(ply, cmd, args)
	if ply:GetTable().Community and ply:GetTable().CommunityRank > 1 then
		...
		local trgEnt = nil
		for _, v in pairs(player.GetAll()) do
			if v:Nick() == trgName then trgEnt = v end
		end
...
function PlySetTitle(ply, cmd, args)
	...
	result = sql.Query(query)
	for _, v in pairs(player.GetAll()) do
		if tostring(v.pid) == pid then v:SetNetVar("ctitle", newTitle) end
	end
end
		...
		if ply:GetTable().CommunityRank > 1 then
			...
			for k, v in pairs(foundStockpiles) do
				if tostring(v:GetNetVar("community_owner")) == tostring(ply:GetTable().Community) then
					...
					timer.Simple(60, function() if IsValid(stockpile) then stockpile:Remove() end end)
					return
				end
			end
		...
		if ply:GetTable().CommunityRank > 1 then
			...
			for _, v in pairs(foundComLockers) do
				if tostring(v:GetNetVar("community_owner")) == tostring(ply:GetTable().Community) then
					...
					timer.Simple(60, function() if IsValid(locker) then locker:Remove() end end)
					return
				end
			end
...
function AddDiplomacy(cid, ocid, status)
	...
	for _, member in pairs(player.GetAll()) do
		if member.Community == cid then member:SetDipl(ocid, status) end
	end
	...
	for _, member in pairs(player.GetAll()) do
		if member.Community == ocid then member:SetDipl(cid, status) end
	end
...
function RemDiplomacy(cid, ocid)
	...
	if result[1]["diplomacy"] then
		...
		result = querySQL(query)
		for _, member in pairs(player.GetAll()) do
			if member.Community == cid then member:SetDipl(ocid, nil) end
		end
	else
	end
	...
	if result[1]["diplomacy"] then
		...
		result = querySQL(query)
		for _, member in pairs(player.GetAll()) do
			if member.Community == ocid then member:SetDipl(cid, nil) end
		end
	else
	end
end
...
function PlyAddDiplomacy(ply, cmd, args)
	...
	if cid == ocid and (status == "war" or status == "ally") then return end
	if ply:GetTable().Community then
		if ply:GetTable().CommunityRank > 1 then
...
function PlyAcptPending(ply, cmd, args)
	...
	if overide == "force" and ply:IsAdmin() then cid = args[4] end
	...
	if self.Community then
		...
		if ocidPending then
			for _, entry in pairs(ocidPending) do
				...
				if dataTbl["cid"] == tostring(cid) and (dataTbl["info"] or "none") == "diplomacy" and (dataTbl["status"] or "none") == "war" then return end
			end
...
function GM:DoPlayerDeath(ply, attacker, dmginfo)
	if ply:GetNetVar("IsAsleep") then ply:ConCommand("pnrp_wake") end
	ply:CreateRagdoll()
	ply:AddDeaths(1)
...
function PNRP.CheckDefWeps(wep)
	...
	for k, v in pairs(defWeps) do
		if string.lower(v) == wep:GetClass() then return true end
	end
end
...
function PNRP.ConvertAmmoType(ammoType)
	...
	if ammoType == 1 then -- Ammunition of the AR2/Pulse Rifle
		sendType = "ar2"
	end

	if ammoType == 2 then -- (name in-game "5.7mm Ammo")
		sendType = "alyxgun"
	end

	if ammoType == 3 then -- Ammunition of the 9MM Pistol
		sendType = "pistol"
	end

	if ammoType == 4 then -- Ammunition of the SMG/MP7
		sendType = "smg1"
	end

	if ammoType == 5 then -- Ammunition of the .357 Magnum
		sendType = "357"
	end

	if ammoType == 6 then -- Ammunition of the Crossbow
		sendType = "xbowbolt"
	end

	if ammoType == 7 then -- Ammunition of the Shotgun
		sendType = "buckshot"
	end

	if ammoType == 8 then -- Ammunition of the RPG/Rocket Launcher
		sendType = "rpg_round"
	end

	if ammoType == 9 then -- Ammunition for the SMG/MP7 grenade launcher (secondary fire)
		sendType = "smg1_grenade"
	end

	if ammoType == 12 then sendType = "sniperround" end
	if ammoType == 22 then -- (name in-game ".45 Ammo")
		sendType = "sniperpenetratedround"
	end

	if ammoType == 10 then -- Note you must be given the grenade weapon (e.g. pl:Give ("weapon_grenade")) before you can throw any grenades
		sendType = "grenade"
	end

	if ammoType == 13 then -- Ammunition cannot exceed 2 (name in-game "Explosive C4 Ammo")
		sendType = "thumper"
	end

	if ammoType == 14 then -- (name in-game "4.6MM Ammo")
		sendType = "gravity"
	end

	if ammoType == 15 then -- (name in-game "9MM Ammo")
		sendType = "battery"
	end

	if ammoType == 16 then sendType = "gaussEnergy" end
	if ammoType == 17 then -- (name in-game ".50 Ammo")
		sendType = "combineCannon"
	end

	if ammoType == 18 then -- (name in-game "5.56MM Ammo")
		sendType = "airboatGun"
	end

	if ammoType == 19 then -- (name in-game "7.62MM Ammo")
		sendType = "striderMinigun"
	end

	if ammoType == 20 then sendType = "helicopterGun" end
	if ammoType == 21 then -- Ammunition of the AR2/Pulse Rifle 'combine ball' (secondary fire)
		sendType = "ar2altfire"
	end

	if ammoType == 11 then -- See Grenade
		sendType = "slam"
	end
	return sendType
end
...
function ConvertWepEnt(weaponModel)
	for itemname, item in pairs(PNRP.Items) do
		if weaponModel == item.Model then return item.Ent end
	end
	return nil
end
...
function PNRP.DropWeapon(ply, command, args)
	...
	if myWep and myWep:GetClass() ~= "weapon_frag" then
		...
		if string.find(myWep:GetModel(), "v_") ~= 1 then wepModel = "models/weapons/w_" .. string.sub(myWep:GetModel(), string.find(myWep:GetModel(), "v_") + 2) end
		if myWep:GetClass() == "weapon_radio" then wepModel = "models/props_citizen_tech/transponder.mdl" end
	...
	if myWep:GetClass() == "weapon_frag" then ply:ChatPrint("Use /dropammo to drop grenades.") end
end
...
function PNRP.DropAmmo(ply, command, args)
	...
	elseif ammoType == "grenade" then
		...
		ammoFType = "ammo_" .. string.lower(ammoType)
	end
	...
	if ItemID then
		...
		local tr = ply:TraceFromEyes(200)
		local trPos = tr.HitPos
		if ammoAmt > ply:GetAmmoCount(ammoType) then ammoAmt = ply:GetAmmoCount(ammoType) end
		ply:ChatPrint("Dropping " .. ammoAmt .. " " .. PNRP.Items[ItemID].Name)
		if ammoType == "slam" or ammoType == "grenade" then
			EntSetting = "ent_weapon"
		else
			EntSetting = PNRP.Items[ItemID].Ent
		end
...
function PNRP.GetSpawnflags(ply)
	...
	if item then
		...
		if item.Hull then ply:ChatPrint("Item Hull: " .. tostring(item.Hull)) end
		if item.Tank then ply:ChatPrint("Gas: " .. tostring(ent.gas) .. "/" .. tostring(item.Tank)) end
		if item.Capacity then ply:ChatPrint("Capacity: " .. tostring(item.Capacity)) end
		if item.HP then HPExt = "/" .. item.HP end
		ply:ChatPrint("Weight: " .. tostring(item.Weight))
	end

	ply:ChatPrint("HP:  " .. tostring(ent:Health()) .. HPExt)
	if ent.UnitLeft then ply:ChatPrint("Charge: " .. tostring(ent.UnitLeft)) end
end
...
hook.Add('EntityKeyValue', "EntityKeyValueInfo", EntityKeyValueInfo)
			...
			if ent:IsPlayerHolding() and ent.ID == "fuel_gas" then
				...
				if IsValid(car) then
					...
					if car.gas < car.tank then
						...
						ent:Remove()
						if car.iid then PNRP.SaveState(ply, ent) end
					else
...
function PNRP.GetPlayerNamePID(pid)
	...
	local result = querySQL(query)
	local name = "Unknown"
	if result then name = result[1]["nick"] end
	return name
end
...
function PNRP.EndRepairTimer(ply, ent)
	...
	net.Send(ply)
	if ent:IsValid() then timer.Stop(ply:UniqueID() .. "_repair_" .. tostring(ent)) end
	ent.Repairing = nil
	ent.BlockF2 = false
	...
	ent.scrapSaved = 0
	ent.ScrapSpent = 0
	if item.SaveState then PNRP.SaveState(nil, ent, "world") end
end
