...
function buy_shop()
	...
	shop_frame.Paint = function() surface.SetDrawColor(50, 50, 50, 0) end
	PNRP.AddMenu(menu)
	...
	PropertySheet.Paint = function() surface.SetDrawColor(50, 50, 50, 0) end
...
function PNRP.build_List(itemtype, parent_frame, PropertySheet)
	...
	if itemtype == "vehicle" and not (ply:IsAdmin() and GetConVarNumber("pnrp_adminCreateAll") == 1) then
		...
		for k, v in pairs(PNRP.CarParts) do
			...
			if not toolItem then continue end
		...
		if item.ShopHide == true and not (ply:IsAdmin() and GetConVarNumber("pnrp_adminCreateAll") == 1) then
			...
			if item.Type == tostring(itemtype) then
				...
				if ply:Team() == TEAM_ENGINEER then
					if item.Scrap ~= nil then
						sc = math.ceil(item.Scrap * (1 - (0.02 * ply:GetSkill("Construction"))))
					else
						sc = 0
					end

					if item.SmallParts ~= nil then
						sp = math.ceil(item.SmallParts * (1 - (0.02 * ply:GetSkill("Construction"))))
					else
						sp = 0
					end

					if item.Chemicals ~= nil then
						ch = math.ceil(item.Chemicals * (1 - (0.02 * ply:GetSkill("Construction"))))
					else
						ch = 0
					end
				else
				...
				if type(neededParts) == "table" then
					...
					for p, n in pairs(neededParts) do
						if PNRP.Items[p] then partsText = partsText .. "\n" .. PNRP.Items[p].Name .. " : " .. tostring(n) end
					end
				end
				...
				if item.Capacity then weightTXT = weightTXT .. " | Capacity: " .. item.Capacity end
				if item.HP then weightTXT = weightTXT .. " | HP: " .. item.HP end
