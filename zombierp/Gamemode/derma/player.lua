...
function viewPlayerInfoWindow()
	...
	player_frame.Paint = function() surface.SetDrawColor(50, 50, 50, 0) end
	...
	function mdl.Entity:GetPlayerColor()
		return targetPly:GetPlayerColor()
	end
...
function playerProfileWindow()
	...
	profile_frame.Paint = function() surface.SetDrawColor(50, 50, 50, 0) end
	...
	profile_TabSheet.Paint = function()
		-- Paint function
	...
	TextName.OnEnter = function(self) chName() end
	...
	NameChangeBtn.DoClick = function() chName() end
		...
		if skill.class ~= nil then
			for classname, class in pairs(PNRP.Skills[skill.name].class) do
				if tostring(ply:Team()) == tostring(class) then
					SkillScrollPanel.ClsSkill = vgui.Create("DLabel", SkillScrollPanel)
	...
	plyWeaponCol.ValueChanged = function() wepColor = plyWeaponCol:GetVector() end
