...
function basicLoadWindow(ply)
	...
	basicload_Frame.Paint = function() surface.SetDrawColor(50, 50, 50, 0) end
	...
	loadinLabel_frame.Paint = function() surface.SetDrawColor(50, 50, 50, 0) end
	...
	Loadin_DPanel.help.DoClick = function() RunConsoleCommand("pnrp_help") end
...
function GM.profilePicker(len)
	...
	for k, v in pairs(plyResult) do
		...
		pnlPanel.Paint = function() draw.RoundedBox(6, 0, 0, pnlPanel:GetWide(), pnlPanel:GetTall(), Color(180, 180, 180, 255)) end
		...
		pnlPanel.delBtn.DoClick = function()
			...
			dvParent.delBtnn.DoClick = function() dvParent:SetVisible(false) end
		end
	...
	for k, v in pairs(AvailableModels) do
		local allowed = true
		if not table.IsEmpty(v.steamID) then if not table.HasValue(v.steamID, ply:SteamID()) then allowed = false end end
	...
	for k, v in pairs(classList) do
		if k > 0 and k < 10 then classlListView:AddLine(team.GetName(k), k) end
	end
...
function PNRP_motd()
	...
	motd_frame.Paint = function() surface.SetDrawColor(50, 50, 50, 0) end
