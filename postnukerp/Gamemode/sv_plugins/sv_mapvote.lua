...
function PNRP_MAPVOTE() -- This is global incase you want to call it for map voting inside PNRP or some other script.
	...
	timer.Create("20sec", tc + 2, 1, function()
		...
		for k, v in pairs(votes) do
			if v > 0 then votesOverZero = votesOverZero + v end
		end
...
local function CheckVote(ply)
	...
	if ply.HasVotedToStart == true then --do not allow voting twice
		pnrp_chat_msg(ply, "You have already voted to change the map!")
		return
	end
	...
	if (math.Round(#player.GetAll() * (#player.GetAll() > 2 and 0.66 or 1)) - Total_Votes) > 0 then if #player.GetAll() > 1 then pnrp_chat_msg_all("(" .. (math.Round(#player.GetAll() * (#player.GetAll() > 2 and 0.66 or 1)) - Total_Votes) .. " more votes(s) needed.)") end end
	CheckVote2()
end
...
local function CloseVote(ply)
	if MasterVoteSwitch == true then
		...
		timer.Simple(CloseTime, function() MasterVoteSwitch = true end)
...
local function ChatVote(ply, text)
	...
	if lastSaid == "/votemap" then
		...
		if ply.HasVotedToStart == false then pnrp_chat_msg(ply, "You voted for a map change!") end
