...
function SQLStr2(str_in, bNoQuotes)
	...
	if null_chr then str = string.sub(str, 1, null_chr - 1) end
	if bNoQuotes then return str end
	return str
end

function querySQL(query)
	result = sql.Query(query)
	if sql.LastError(result) ~= nil and result == false then ErrorNoHalt(tostring(os.date()) .. "[SQL ERROR] \n QUERY: " .. query .. " \n Error:  " .. tostring(sql.LastError()) .. "\n") end
	return result
end
...
function setServerSetting(setting, value)
	...
	result = querySQL(query)
	--Sets these Convars
	if GetConVar("pnrp_" .. setting) then RunConsoleCommand("pnrp_" .. setting, tostring(value)) end
	return result
end
...
function runUpgradeCheck(ply, command, args)
	if ply:IsAdmin() then
		PNRP.UpgradeCheck()
		if ply then ply:ChatPrint("Upgrade Check Run") end
...
--End the "when player first joins server and spawns" function
...
function GM:PlayerSpawn(ply)
	...
	if ply:GetTable().Hunger == 0 then ply:GetTable().Hunger = 100 end
	...
	ply:IncResource("Small_Parts", 0)
	ply:IncResource("Chemicals", 0)
	if not ply:GetTable().SleepGodCheck then
...
function GM:PlayerDisconnected(ply)
	...
	if plUID == "None" then plUID = ply:UniqueID() end
	if ply.HasLoaded then self.SaveCharacter(ply) end
	PNRP.GetAllCars(ply)
	...
	timer.Create(tostring(ply:UniqueID()) .. "plyCK", 60, 1, function()
		...
		for k, v in pairs(player.GetAll()) do
			if v:Nick() == TMPPlayerName then PlayerOnCheck = true end
		end

		if not PlayerOnCheck then
			local OwnedList = PNRP.ListOwnedItems(plUID)
			for k, v in pairs(OwnedList) do
				if IsValid(v) then
					local skip = false
					local itmID = PNRP.FindItemID(v:GetClass())
					if itmID then if PNRP.Items[itmID].Persistent then skip = true end end
					if not skip then
						v:SetNetVar("Owner", "World")
						v:SetNetVar("Owner_UID", "None")
...
--Here we end the Loadout function
...
function PNRP.ClassIsNearby(pos, class, range)
	...
	for k, v in pairs(ents.FindInSphere(pos, range)) do
		if v:GetClass() == class and (pos - Vector(v:LocalToWorld(v:OBBCenter()).x, v:LocalToWorld(v:OBBCenter()).y, pos.z)):Length() <= range then nearby = true end
	end
	return nearby
end
...
function GM.run_Command(ply, command, arg)
	if ply:IsAdmin() then RunConsoleCommand(arg[1], arg[2]) end
end
...
function GM:ShowTeam(ply)
	...
	if ent:GetClass() == "msc_itembox" or ent:GetClass() == "msc_display_item" then ent:F2Use(ply) end
		...
		if myType == "vehicle" then
			...
			if myType == "weapon" then
				...
				if weight <= weightCap then
					...
					if tonumber(ent:GetNetVar("Ammo", 0)) > 0 and not (ItemID == "wep_grenade" or ItemID == "wep_shapedcharge") then ply:GiveAmmo(tonumber(ent:GetNetVar("Ammo")), PNRP.FindAmmoType(ItemID, nil)) end
					ent:Remove()
				else
			...
			elseif myType == "ammo" then
				...
				if weight <= weightCap then
					PNRP.AddToInventory(ply, ItemID, 1, ent)
					if myType == "tool" or myType == "misc" then PNRP.TakeFromWorldCache(ply, ItemID) end
					ent:Remove()
				else
					ply:ChatPrint("Your pack is too full and cannot carry this.")
				end
...
function GM:ShowSpare1(ply)
	...
	if tostring(ent) ~= "[NULL Entity]" then
		...
		if item ~= nil then
			if tostring(ent:GetNetVar("Owner_UID", "None")) == PNRP:GetUID(ply) and item.HasStorage then
				if ent.iid then
...
function PNRP.GetAllTools(ply)
	for k, v in pairs(ents.GetAll()) do
		...
		if ItemID ~= nil then
			local myType = PNRP.Items[ItemID].Type
			if tostring(v:GetNetVar("Owner_UID", "None")) == PNRP:GetUID(ply) and (myType == "tool" or myType == "misc") then
...
function PNRP.GetAllCars(ply)
	for _, car in pairs(ents.GetAll()) do
		...
		if item then
			if tostring(car:GetNetVar("Owner_UID", "None")) == PNRP:GetUID(ply) and item.Type == "vehicle" then
...
function PNRP.GetCar(ply)
	...
	if tostring(car) ~= "[NULL Entity]" then
		...
		if ItemID ~= nil then
			local myType = PNRP.Items[ItemID].Type
			if tostring(ent:GetNetVar("Owner_UID", "None")) == PNRP:GetUID(ply) and item.Type == "vehicle" then
				local ItemID = item.ID
				Msg("Sending " .. ItemID .. " to " .. ply:Nick() .. "'s Inventory" .. "\n")
				if car.iid then
					PNRP.SaveState(ply, car, "player")
	...
	if Victim:IsPlayer() and Attacker:IsPlayer() then
		...
		if Victim ~= Attacker then
			...
			for i, ent in ipairs(found_ents) do
				if ent:GetClass() == "info_player_start" or ent:GetClass() == "info_player_terrorist" or ent:GetClass() == "info_player_counterterrorist" then victumFoundSpawn = true end
			end
			...
			for i, ent in ipairs(found_ents) do
				if ent:GetClass() == "info_player_start" or ent:GetClass() == "info_player_terrorist" or ent:GetClass() == "info_player_counterterrorist" then attackerFoundSpawn = true end
			end
...
function EntTakeDamage(target, dmginfo)
	...
	--
	if attacker:IsVehicle() then if target:GetClass() == "npc_combine_s" then dmginfo:SetDamage(0) end end
	...
	if attacker:IsPlayer() then
		if attacker:InVehicle() then
			...
			for _, seat in pairs(conEnts) do
				if seat.Entity[2].Entity == attacker:GetVehicle() then dmginfo:SetDamage(0) end
			end

			if inTable(conEnts, target) then ply:ChatPrint("ping") end
			if target == ply:GetVehicle() then dmginfo:SetDamage(0) end
		end
	end

	local dmgEnt = target
	if target:IsPlayer() then if target:InVehicle() then dmgEnt = target:GetVehicle() end end
	local targetItem = PNRP.SearchItembase(dmgEnt)
	if targetItem then
		if targetItem.CanRepair then
			local damage = dmginfo:GetDamage()
			dmgEnt:SetHealth(dmgEnt:Health() - damage)
			if dmgEnt:Health() <= 0 then dmgEnt:SetHealth(0) end
		end
	end
