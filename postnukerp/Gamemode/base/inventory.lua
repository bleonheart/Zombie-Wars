...
function PNRP.Inventory(ply)
	...
	for i, invStr in pairs(invLongStr) do
		...
		if count > 0 and itemid ~= "" then invTbl[invSplit[1]] = count end
	end
	return invTbl
end
...
function PNRP.StorageInventoryWeight(iid)
	...
	for k, v in pairs(Inv) do
		if PNRP.Items[v["itemid"]] then if PNRP.Items[v["itemid"]].Type ~= "vehicle" then weightSum = weightSum + PNRP.Items[v["itemid"]].Weight * tonumber(v["count"]) end end
	end
	return weightSum
end
...
function PNRP.InventoryWeight(ply)
	...
	for ID, count in pairs(Inv) do
		if PNRP.Items[ID] then if PNRP.Items[ID].Type ~= "vehicle" then weightSum = weightSum + (PNRP.Items[ID].Weight * tonumber(count)) end end
	end

	for k, v in pairs(Inv2) do
		if v["location"] == "player" and PNRP.Items[v["itemid"]] then if PNRP.Items[v["itemid"]].Type ~= "vehicle" then weightSum = weightSum + PNRP.Items[v["itemid"]].Weight end end
	end
	return weightSum
end
...
function PNRP.CheckPlayerWeight(ply, theitem)
	...
	if weight <= weightCap then return true end
	return false
end
...
function PNRP.PlyInvAddToItemStorage(len, ply)
	...
	if origin_iid then PNRP.OpenItemInventory(origin_iid, ply) end
end
...
function PNRP.DropPersistItem(ply, itemID, iid, option)
	...
	if not item.SaveState then PNRP.DelPersistItem(iid) end
...
function PNRP.PersistMoveTo(ply, iid, location, locid, locdata)
	...
	if iid then
		...
		result = querySQL(query)
		if IsEntity(ply) then if ply:IsPlayer() then ply:EmitSound(Sound("items/ammo_pickup.wav")) end end
	end
...
function PNRP.DropItem(p, c, a)
	...
	for itemname, item in pairs(PNRP.Items) do
		for k, v in pairs(inv) do
			if k == itemname and k == a[1] then PNRP.DropSpawn(p, itemname, 1) end
		end
	end
end
...
function PNRP.DropCarItem(p, c, a)
	...
	for itemname, item in pairs(PNRP.Items) do
		for k, v in pairs(inv) do
			if k == itemname and k == a[1] then PNRP.DropCarSpawn(p, itemname, 1) end
		end
	end
end
...
function PNRP.UseFromInvStoreage(len, ply)
	...
	if usedFrom == "storage" then
		...
		if item.Type == "weapon" or item.Type == "ammo" or item.Type == "medical" or item.Type == "food" then
			local useCheck
			useCheck = item.Use(ply)
			if useCheck == true then PNRP.TakeFromItemStorage(sid, ItemID, 1) end
		else
